...
_SOKOL_PRIVATE bool _saudio_alsa_backend_init(void) {
    ...
    */
    ...
    }
    {
        ...
        int policy = SCHED_RR; // Round Robin scheduling policy

        // Set priority (range is typically from 1 to 99 for real-time policies)
        param.sched_priority = 20; // Choose an appropriate priority

        // Set the scheduling policy and priority for the audio thread
        if (0 != pthread_setschedparam(_saudio.backend.thread, policy, &param)) {
